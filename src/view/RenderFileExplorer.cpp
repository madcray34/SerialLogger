...
void FileExplorer::Draw(std::string_view label)
{
    ...
    ImGui::SetCursorPosY(ImGui::GetWindowHeight() -
                         100.00F); //set next element to position
    ImGui::Separator();
    DrawActions();
    ImGui::Separator();
    DrawFilter();
...
void FileExplorer::DrawMenu()
{
    ...
    ImGui::PushTextWrapPos(ImGui::GetContentRegionAvail()
                               .x); // Enable wrapping at the window's max width
...
void FileExplorer::DrawContent()
{
    for (const auto &entry : fs::directory_iterator(m_currentPath))
    {
        ...
        //The boolean is for the refrashe rate to mantain highlighted the last selected item in the current view
